...
SELECT coalesce((SELECT 'intent='||intent.state FROM public.payment_intents AS intent WHERE intent.reservation_id='$ReservationID'::uuid ORDER BY intent.created_at DESC LIMIT 1),'intent=missing')||'|'||
       ...
       coalesce((SELECT 'actions='||string_agg(action.action_type||':'||action.state,',' ORDER BY action.action_type) FROM public.payment_saga_actions AS action JOIN public.payment_sagas AS saga ON saga.saga_id=action.saga_id JOIN public.payment_intents AS intent ON intent.payment_intent_id=saga.payment_intent_id WHERE intent.reservation_id='$ReservationID'::uuid),'actions=none')
"@
