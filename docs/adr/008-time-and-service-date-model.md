...
- Materialize `scheduled_departure_at` as a UTC instant when creating the train run. Arrival/departure instants are derived from the service date, route timezone, and offsets, then converted to UTC.
...
Hold expiry is an absolute UTC instant. Confirmation requires the locked reservation to remain `held` and `now < expires_at`. Expiration requires `held` and `now >= expires_at`.
