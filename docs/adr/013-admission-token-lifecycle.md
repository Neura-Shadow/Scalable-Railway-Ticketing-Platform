...
The opaque raw token is the versioned base64url encoding of the key ID, delivery nonce, and issuance MAC. The derivation key is exactly 32 bytes of external secret material shared only by API and admission-worker processes. It has no development or production default, is never logged, and is validated as process-owned configuration. The worker stores only `SHA-256(raw token)`, the random nonce, immutable signed context, issuance MAC, binding metadata, state, and expiry indexes. Redis never stores the raw token or an encrypted raw-token copy.
...
The customer status script atomically verifies JWT-derived ownership and a still-undelivered admitted entry, returns the signed issuance fields once, and marks delivery complete in the same operation. The API validates the MAC and then assembles and returns the raw token. Later status calls cannot return it. The nonce remains only as signed verification metadata until token TTL so acquire can validate the envelope; the delivery flag, not nonce deletion, enforces one response. Nonces, raw tokens, token hashes, and issuance MACs are never logged or used as metric labels.
